#!/bin/sh
# Switch Impala to use debug binaries
...
sudo service hive-metastore stop
 ...
 # Use the debug version of the course impala defaults script

if [ -L "/etc/default/impala" ]; then
    sudo rm /etc/default/impala
    sudo ln -s /etc/default/impala.training.debug /etc/default/impala
else
...
echo "Impala now running in debug mode"